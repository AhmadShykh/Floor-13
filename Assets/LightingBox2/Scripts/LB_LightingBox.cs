...
public class LB_LightingBox : EditorWindow
{
	...
	*/
	...
	void OnGUI()
	{
		...
		GUILayout.Label ("Lighting Box 2 - ALIyerEdon@gmail.com - v2.8 march 2021", EditorStyles.helpBox);
		...
		*/
		...
		if (winMode == WindowMode.Part1) {
			...
			if(vLightState)
			{
				if (helpBoxVideo)
				{
					if (GUILayout.Button("Watch -Volumetric Light- video guide"))
						Application.OpenURL(lb_guide_videos["Guide Videos"]["Volumetric Light"].StringValue);
				}
				if (helpBox)
			...
			if(snowState)
			{
				...
				if (helpBoxVideo)
				{
					if (GUILayout.Button("Watch -Snow- video guide"))
						Application.OpenURL(lb_guide_videos["Guide Videos"]["Snow"].StringValue);
				}
			}
			...
			if(aoState)
			{
				...
				if (helpBoxVideo)
				{
					if (GUILayout.Button("Watch -AO- video guide"))
						Application.OpenURL(lb_guide_videos["Guide Videos"]["AO"].StringValue);
				}
			}
			...
			{
				if (helpBox)
					EditorGUILayout.HelpBox ("Activate Stochastic Screen Space Reflections effect for your camera, read the doc files for its source", MessageType.Info);
				if (helpBoxVideo)
				{
					if (GUILayout.Button("Watch -Stochastic SSR- video guide"))
						Application.OpenURL(lb_guide_videos["Guide Videos"]["Stochastic SSR"].StringValue);
				}
			}
